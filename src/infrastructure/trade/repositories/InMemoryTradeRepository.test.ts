...
describe('InMemoryTradeRepository', () => {
  ...
  it('saves a domain Trade (via TradeFactory.toDTO) and then returns it in getAll', async () => {
    const trade = TradeFactory.create({ id: 't-save', symbol: 'TEST1', entryDate: new Date().toISOString(), size: 1, price: 10, side: 'LONG' })
  ...
  it('updates an existing trade', async () => {
    ...
    const modified = TradeFactory.create({ id: t.id, symbol: t.symbol.value, entryDate: t.entryDate.value, size: t.size.value, price: originalPrice + 1, side: t.side.value })
  ...
  it('toRepoTrade accepts VO-like objects and primitive shapes', async () => {
    ...
    const fakeTrade = TradeFactory.create({ id: 'vo1', symbol: 'VOUSD', entryDate: new Date().toISOString(), size: 2, price: 5, side: 'LONG' })
