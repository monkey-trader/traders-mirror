...
describe('RepoSyncStatus interactions', () => {
  it('renders local by default and shows queued when event dispatched', async () => {
    ...
    act(() => {
      globalThis.dispatchEvent(
        new CustomEvent('repo-sync-status', { detail: { feature: 'trade', status: 'queued', queuedCount: 3 } })
      )
    })
  ...
  it('starts syncing on click and responds to online completion', async () => {
    ...
    act(() => {
      globalThis.dispatchEvent(new CustomEvent('repo-sync-status', { detail: { feature: 'trade', status: 'online' } }))
    })
