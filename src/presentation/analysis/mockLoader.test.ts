...
describe('mockLoader', () => {
  ...
  it('uses repo.listAll() when available and calls save for created analyses', async () => {
    ...
    const repo: {
      ...
      listAll: vi.fn(async () => [
        { id: 'repo-1', symbol: 'X', createdAt: new Date().toISOString(), notes: 'from-repo' },
      ]),
      seed: vi.fn(),
    };
    ...
    expect(setAnalyses).toHaveBeenCalledWith([
      { id: 'repo-1', symbol: 'X', createdAt: expect.any(String), notes: 'from-repo' },
    ]);
  });
  ...
  it('clearAnalyses prefers clear() then falls back to listAll+delete', async () => {
    ...
    await clearAnalyses(
      repoWithClear as unknown as Parameters<typeof clearAnalyses>[0],
      setAnalyses
    );
    ...
    await clearAnalyses(
      repoFallback as unknown as Parameters<typeof clearAnalyses>[0],
      setAnalyses
    );
