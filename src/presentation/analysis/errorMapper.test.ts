...
describe('analysis errorMapper', () => {
  ...
  it('maps domain error prototypes to German messages when not a plain validation-like object', () => {
    ...
    expect(mapDomainErrorToUI(protoSymbol)).toEqual({ field: 'symbol', message: 'Symbol ist ungültig' })
    ...
    expect(mapDomainErrorToUI(protoDate)).toEqual({ field: 'entryDate', message: 'Datum ist ungültig' })
    ...
    expect(mapDomainErrorToUI(protoTv)).toEqual({ field: 'tradingViewLink', message: 'TradingView Link ist ungültig' })
