...
describe('TradeFilters', () => {
  it('MarketFilters calls setMarketFilter and shows trades count', () => {
    ...
    const { container } = render(<MarketFilters marketFilter="Crypto" setMarketFilter={setMarketFilter} tradesCount={5} />)
  ...
  it('StatusFilters calls setTradeStatusFilter', () => {
    ...
    const { container } = render(<StatusFilters tradeStatusFilter="OPEN" setTradeStatusFilter={setTradeStatusFilter} />)
    const root = container.firstChild as HTMLElement
    fireEvent.click(within(root).getByText('All'))
  ...
  it('default composite TradeFilters renders both parts and forwards callbacks', () => {
    const props = { marketFilter: 'All' as const, setMarketFilter: vi.fn(), tradeStatusFilter: 'ALL' as const, setTradeStatusFilter: vi.fn(), tradesCount: 2 }
