...
Dieser Abschnitt dokumentiert die wiederverwendbaren Presentation-Komponenten, die unter `src/presentation/shared/components` liegen. Ziel ist eine kompakte API-Referenz (Props), Hinweise zur Implementation und zum Testen sowie ein visuelles Diagramm, das ihre Rolle im Systems zeigt.
...
- ErrorMapper (`src/presentation/trade/errorMapper.ts`) – Präsentations-Utility zum Mappen typisierter Domain-Errors auf Feld- oder globale Fehlermeldungen (i18n-ready).
...
== ErrorMapper
Pfad: `src/presentation/trade/errorMapper.ts`

Beschreibung::
- Utility in der Presentation-Schicht, das typisierte Domain-Fehler (`DomainErrors`) auf Präsentations-Feldfehler (z. B. `size`) oder eine globale, i18n-fähige Fehlermeldung mapped.
- Hält die `TradeJournal.tsx`-Komponente schlank und testbar.

=== Verhalten
- Input: `err: unknown` (z. B. DomainError-Instanz)
- Output: `{ fieldErrors?: Partial<Record<string, string>>; message?: string }`
- Bekannte Domain-Fehler liefern `fieldErrors`, unbekannte Fehler liefern eine generische `message`.

=== Tests
- Unit-Tests prüfen, dass jede Domain-Error-Klasse korrekt auf das erwartete Feld gemappt wird und dass unbekannte Fehler eine allgemeine Nachricht zurückgeben.
